  ...
  async execute(message, args, parseCmd) {
    ...
    switch (true) {
      ...
      case Array.isArray(channelsToMoveTest): {
        // array of channels
        channelsToMove = guild.channels.cache.filter((channel) =>
          channelsToMoveTest.includes(channel.name.toLowerCase())
        );
        break;
      }
    }
    ...
    if (beforeChangeArrFirstHalf.length > 0)
      ...
      switch (reaction.emoji.name) {
        case "✅": {
          ...
          channelsToMove.forEach((channel) => {
            channel.setParent(destCategory.id);
            if (++i >= channelAmount)
              eventEmitter.emit(`move_channels-${timestamp}`);
          });
          break;
        }
        case "❌":
