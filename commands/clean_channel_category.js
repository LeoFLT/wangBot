      ...
      switch (reaction.emoji.name) {
        case '✅': {
          ...
          eventEmitter.once(`clean_channel_category-${timestamp}`, async () => {
            ...
            finalCategory.children.forEach((channel) => finalChannelArr.push(channel));
            ...
            if (channelErrArr.length > 0) finalMessage.addField('Errored channels: ', channelErrArr, false);
          ...
          category.children.forEach(async (channel) => {
            ...
            if (++i >= channelAmount) eventEmitter.emit(`clean_channel_category-${timestamp}`);
          });
          break;
        }
        case '❌':
